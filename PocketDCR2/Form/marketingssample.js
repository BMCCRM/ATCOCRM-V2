...
function onSuccessG1(data, status) {
    ...
    $("#dG2").append("<option style='height:20px;' value='" + value + "'>" + name + "</option>");

    $.each(data.d, function (i, tweet) {
        $("#dG2").append("<option style='height:20px;'  value='" + tweet.Item1 + "'>" + tweet.Item2 + "</option>");
    });
...
function onSuccessG2(data, status) {
    ...
    $("#dG3").append("<option style='height:20px;'  value='" + value + "'>" + name + "</option>");

    $.each(data.d, function (i, tweet) {
        $("#dG3").append("<option style='height:20px;'  value='" + tweet.Item1 + "'>" + tweet.Item2 + "</option>");
    });
...
function onSuccessG3(data, status) {
    ...
    $("#dG4").append("<option style='height:20px;' value='" + value + "'>" + name + "</option>");

    $.each(data.d, function (i, tweet) {
        $("#dG4").append("<option style='height:20px;' value='" + tweet.Item1 + "'>" + tweet.Item2 + "</option>");
    });
...
function onSuccessFillGH(data, status) {
    ...
    $("#dG1").append("<option style='height:20px;' value='" + value + "'>" + name + "</option>");

    $.each(data.d, function (i, tweet) {
        $("#dG1").append("<option style='height:20px;' value='" + tweet.Item1 + "'>" + tweet.Item2 + "</option>");
    });
...
//    try {
        ...
        myData = "{'level3':'" + level3 + "','level4':'" + level4 + "','level5':'" + level5 + "','level6':'" + level6 + "','txtDate1':'" + date1 + "','txtDate2':'" + date2 + "'}";

        $.ajax({
            type: "POST",
            url: "marketingplansample.asmx/GetProduct",
            contentType: "application/json; charset=utf-8",
            data: myData,
            dataType: "json",
...
function onSuccessGridFill(data) {
        ...
        else if (jsonObj[0].level == "Level3") {
            $('#gridproduct').prepend($('<table id="datatables" class="column-options"><thead><tr><th Class="ob_gC_Fc">Sample Product</th><th Class="ob_gC_Fc">Sample Issue</th><th Class="ob_gC_Fc">Previous Balance</th><th Class="ob_gC_Fc">Total</th><th Class="ob_gC_Fc">Sample Distributed</th><th Class="ob_gC_Fc">Current Month Balance</th></tr></thead><tbody>'));
            $.each(jsonObj, function (i, option) {
                $('#datatables').append($("<tr><td class='ob_text'>" + option.ProductName + "</td><td class='ob_text'>" + option.IssueQTY + "</td><td class='ob_text'>" + option.PreviousBalance + "</td><td class='ob_text'>" + option.Total + "</td><td class='ob_text'>" + option.Balance + "</td><td class='ob_text'>" + option.CurMonthTotal + "</td>"
                     + '</tr>'));
            });
        }
        else {
           
            $('#gridproduct').prepend($('<table id="datatables" class="column-options"><thead><tr><th Class="ob_gC_Fc">Sample Product</th><th Class="ob_gC_Fc">Sample Issue</th><th Class="ob_gC_Fc">Previous Balance</th><th Class="ob_gC_Fc">Total</th><th Class="ob_gC_Fc">Sample Distributed</th><th Class="ob_gC_Fc">Current Month Balance</th><th Class="ob_gC_Fc">Enter Received Quantity</th><th Class="ob_gC_Fc">Save</th></tr></thead><tbody>'));
            $.each(jsonObj, function (i, option) {
                $('#datatables').append($("<tr><td class='ob_text'>" + option.ProductName + "</td><td class='ob_text'>" + option.IssueQTY + "</td><td class='ob_text'>" + option.PreviousBalance + "</td><td class='ob_text'>" + option.Total + "</td><td class='ob_text'>" + option.Balance + "</td><td class='ob_text'>" + option.CurMonthTotal + "</td><td class='ob_text'>"
                    + "<input type='text' id='ProQty" + i + "' name='ProQty'>" + "</td>"
                     + '<td><a id="button" href="#" onclick="oGrid_Edit(' + option.ProductId + ',' + i + ');return false">Save</a>' + '</td></tr>'));
            });
        }

       
    
//    //  $('#gridproduct').append($('</tbody></table>'));style='width: 800px;'
//    // var table = $('gridproduct').DataTable();
//    if (data.d != 'No') {
//        jsonObj = jsonParse(data.d);
...
//        //$("#gridproduct").empty();
//        //$("#gridproduct").prepend($("<table id='datatabl'><thead>" +
//        //                    "<tr>" +
//        //                    "  <th>SKU ID</th>" +
...
function oGrid_Edit(proid,rowid) {
//    $('#dialog').jqm({ modal: true });
//    $('#dialog').jqm();
//    $('#dialog').jqmShow();
...
//    }
    ...
    var qtydata = $('#ProQty' + rowid).val();
    if (qtydata == "")
    {
        alert("Please Insert Quantity");
        return false;
    }
    if (qtydata !== parseInt(qtydata,10)) {
        alert("Please only enter numeric Number only! (Allowed input:0-9)")
        return false;
    }
    ...
    //if (level6 == '-1' || level6 == null || level6 == 0) {
    //    alert("Please Select MedRep !");
    //    return false;
    //}

    // myData = "{'l1':'" + level3 + "','l2':'" + level4 + "','l3':'" + level5 + "','l4':'" + level6 + "'}";
    myData = "{'ProductID':'" + proid + "','QTY':'" + qtydata + "','txtDate1':'" + date1 + "','txtDate2':'" + date2 + "','level6':'" + level6 + "','level5':'"+level5+"','level4':'"+level4+"','level3':'"+level3+"'}";
...
var onSuccessProductQtyInsert = function (response) {
    if (response == "Error") {
