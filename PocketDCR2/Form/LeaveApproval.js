...
function GetDataInGrid(date) {
    ...
    try {
        ...
        $.ajax({
            type: "POST",
            url: "LeaveApproval.asmx/GetLeaveApproval",
...
function onSuccessGridFill(response, data, status) {
    ...
    if (response.d != 'No') {
        ...
        for (var i = 0; i < jsonObj.length ; i++) {
            ...
            if (jsonObj[i].ApprovalStatus == 'Approved' || jsonObj[i].ApprovalStatus == 'Rejected') {
                apro = "<td><a class='buttonlink' onClick=\"oGrid_AddTolistApprove('" + jsonObj[i].PkId + "');\" href='javascript:;' style='vertical-align: text-top; display: inline-block; disabled' >Approved</a>"
            }
            else {
                apro = "<td><a class='buttonlink' onClick=\"oGrid_AddTolistApprove('" + jsonObj[i].PkId + "');\" href='javascript:;' style='vertical-align: text-top; display: inline-block;' >Approved</a>"
                    ...
                    + "<td><a class='buttonlinkRed' onClick=\"oGrid_AddTolistRejected('" + jsonObj[i].PkId + "');\" href='javascript:;' style='vertical-align: text-top; display: inline-block;' >Reject</a>"
            ...
            "<td>" + jsonObj[i].EmployeeCode + "</td>" +
            "<td>" + jsonObj[i].EmployeeName + "</td>" +
            "<td>" + jsonObj[i].ActivityType + "</td>" +
            "<td>" + jsonObj[i].FromDate + "</td>" +
            "<td>" + jsonObj[i].ToDate + "</td>" +
            "<td>" + jsonObj[i].RequestDate + "</td>" +
            "<td>" + jsonObj[i].ApprovalStatus + "</td>"
        ...
        $("<div title='Alert'>No approval requests were sent to change leave.</div>").dialog();
    }
}
...
function (isConfirm) {
    if (isConfirm) {
        ...
        var mydata = "{'in_ids':'" + ids.toString() + "','Status':'0'}";

        $.ajax({
            type: "POST",
            url: "LeaveApproval.asmx/allPostApproval",
            data: mydata,
            contentType: "application/json; charset=utf-8",
            success: function (data) {
                jsonObj = $.parseJSON(data.d);
                if (jsonObj[0].Msg == "OK") {
                    sweetAlert('success', 'Leave has been changed successfully', 'success');
                }
                else {
                    sweetAlert('error', 'Leave has been not changed successfully', 'error');
                }
                GetDataInGrid('01 ' + $("#datemonth").val());
            },
            beforeSend: startingAjax,
...
function (isConfirm) {
    if (isConfirm) {
        ...
        var mydata = "{'in_ids':'" + ids.toString() + "','Status':'1'}";
...
function LeaveCompare(nlat, nlng, nadd, plat, plng, padd) {
    $("#dialog1").dialog({
        modal: true,
        width: 800
    });
...
function initMap(nlat, nlng, nadd, plat, plng, padd) {
    ...
    var contentString = '<div id="content">' +
            '<div id="siteNotice">' +
            '</div>' +
            '<h1 id="firstHeading" class="firstHeading">Previous Leave</h1>' +
            '<div id="bodyContent">' +
    ...
    var contentString1 = '<div id="content">' +
           '<div id="siteNotice">' +
           '</div>' +
           '<h1 id="firstHeading" class="firstHeading">New Leave</h1>' +
           '<div id="bodyContent">' +
    ...
    //    var marker = new google.maps.Marker({
    //        position: new google.maps.LatLng(Leaves[i][1], Leaves[i][2]),
    //        map: map,
    //        title: 'loc'
    //    });
