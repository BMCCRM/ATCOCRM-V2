...
function pageLoad() {
    ...
    showplan();
    var returnofmioplan1 = mioplan();

    mioPlan2(returnofmioplan1.split('|')[0], returnofmioplan1.split('|')[1], returnofmioplan1.split('|')[2], returnofmioplan1.split('|')[3], returnofmioplan1.split('|')[4]);
    FillProductSku();
    FillGifts();
    loadbrick();
    loadClass();
...
function mioplan() {
    ...
    var monthRepresentation = result;
    var ddata = Gdate.toDateString();
    var mmoth = Gdate.getMonth() + 1;

    var myear = Gdate.getFullYear();;
    var myDatamioplan1 = '';
    if (CurrentUserRole == "rl6") {
        myDatamioplan1 = "{'_date':'" + ddata + "','_EmployeeId':'" + EmployeeId + "'}";
    }
    else if (CurrentUserRole == "rl5") {

        myDatamioplan1 = "{'_date':'" + ddata + "','_EmployeeId':'" + $('#selmio').val() + "'}";
    }

    return myDatamioplan1 + "|" + mmoth + "|" + myear + "|" + ddata + "|" + monthRepresentation;


}


function mioPlan2(myDatastring, mmoth, myear, ddata, monthRepresentation) {
    var savedata = false;
    var GPlanStatus = "";
    $.ajax({
        type: "POST",
        ...
        success: function (data) {
            if (data.d != null) {
                ...
                if (CurrentUserRole == "rl6") {
                    ...
                    var myDataforRL6 = "{'_date':'" + ddata + "','_DoctorId':'" + Drid + "','_ClassId':'" + Classid + "','_BrickId':'" + Brickid + "','_SpecialityId':'" + Speid
                      + "','_EmployeeId':'" + EmployeeId + "'}";

                    //calling mioPlan3ForRL6
                    mioPlan3ForRL6(myDataforRL6, monthRepresentation, mmoth, myear, savedata);
                            ...
                            $.ajax({
                                ...
                                success: function (data) {
                                    ...
                                    $.each(result1, function (i, option) {
                                        ...
                                        $('#' + ii).append("<li><div style='width: 180px;height: 10px;padding: 10px;background-color:#ccc'>" + drname + "</div></li>");
                                        $('#' + ii).append("<li><div style='width: 45px;height: 10px;padding: 10px;background-color:#ccc'>" + drclass + "</div></li>");
                                        $('#' + ii).append("<li><div style='width: 55px;height: 10px;padding: 10px;background-color:#ccc'>" + specialty + "</div></li>");
                                        $('#' + ii).append("<li><div style='width: 165px;height: 10px;padding: 10px;background-color:#ccc'>" + brick + "</div></li>");
                                        $('#' + ii).append("<li><div style='width: 30px;height: 10px;padding: 10px;background-color:#ccc'>" + Target + "</div></li>");
                                        $('#' + ii).append("<li><div style='width: 30px;height: 10px;padding: 10px;background-color:#ccc'>" + Actual + "</div></li>");
                                        $('#' + ii).append("<li><div style='width: 30px;height: 10px;padding: 10px;background-color:#ccc'>" + Pending + "</div></li>");
                                        //$('#' + ii).append("<li><div style='width: 30px;height: 10px;padding: 10px;background-color:#ccc'>" + Total + "</div></li>");
...
function mioPlan3ForRL6(myDataString, monthRepresentation, mmoth, myear, savedata) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $.each(result1, function (i, option) {
                ...
                $('#' + ii).append("<li style='width: 190px;padding: 0 5px 5px;background-color:#ccc'>" + drname + "</li>");
                ...
                for (var i = 0; i < monthRepresentation.length; i++) {
                    ...
                    if (checkedoption == "True") {
                        ...
                        if (i < 10) {
                            callcountid = i + 1;
                            callcountid = "0" + callcountid;
                            brdaydata = parseInt($('#' + callcountid + "_brday").text());
                            $('#' + callcountid + "_brday").text(brdaydata + 1);
                        } else {
                            callcountid = i + 1;
                            brdaydata = parseInt($('#' + callcountid + "_brday").text());
        ...
        success: function (data) {
            ...
            $.each(result1, function (i, option) {
                ...
                if (_PlanStatus == "Approve") {
                    ...
                    exu = "<div class='box2' onclick='FillForm_dayview(" + planid + ")'></div>";
