    ...
    $.fn.simsCheckbox = function (options) {
        ...
        return selectorElt.each(function () {
            ...
            simsThis.find(settings.element).each(function (i) {
                var simsElement = $(this),
                simsElementTitle = simsElement.html();
                //add checkbox class
            ...
            if (settings.selectAllBtn) {
                ...
                var selectAllBtnContainer = $('<ul></ul>').css({
                    'margin': '5px 0 0 0',
                    'padding': '0'
                });
                var selectAllBtnElt = $('<' + settings.element + ' class="sims-btn-select-all btn-block"></' + settings.element + '>').css({
                    'border': '1px dashed',
                    'text-align': 'left'
                }).addClass(checkboxClass + ' ' + (allChecked ? settings.checkedClass : settings.uncheckedClass)).html('<i class="fa fa-fw fa-' + (allChecked ? 'check-' : '') + settings.titleIcon + '"></i> ' + settings.selectAllText);
                simsThis.before(selectAllBtnContainer.append(selectAllBtnElt));
                ...
                selectAllBtnContainer.find('.sims-btn-select-all').off('click').on('click', function (e) {
                    ...
                    simsThis.find(settings.element + '.sims-selectable:not(.disabled)').each(function () {
                        ...
                        if (thisButton.hasClass('btn-info')) $(this).removeClass(settings.uncheckedClass).addClass(settings.checkedClass).find('i:first-child').removeClass('fa-' + settings.titleIcon).addClass('fa-check-' + settings.titleIcon);
