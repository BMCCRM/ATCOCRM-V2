...
(function ($) {
    ...
    Listbox.prototype._createListbox = function () {
        this._listbox = $('<div id="lst' + iDoflist + '">')
            .addClass(MAIN_CLASS)
    ...
    Listbox.prototype.addItem = function (parentItem) {
        var self = this;
        var valueofoption = parentItem.val();
        var item = $('<div id="' + valueofoption + '">')
    ...
    SingleSelectListbox.prototype.onItemClick = function (item) {
        ...
        $.ajax({
            type: 'POST',
            url: 'MRProducts.asmx/GetAllProductsofMedRep',
            contentType: 'application/json; charset=utf-8',
            dataType: "json",
            data: "{'medRepId':'" + item.attr('id') + "'}",
            async: false,
            success: function (data) {

                $("#lstproducts .lbjs-list > div").removeAttr('selected');

                var response = jsonParse(data.d);
                $.each(response, function (i, tweet) {
                    $("#lstproducts").find("#" + tweet.SkuId).attr('selected', '');
                });
