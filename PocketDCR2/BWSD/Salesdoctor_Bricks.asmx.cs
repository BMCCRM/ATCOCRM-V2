...
{
    /// <summary>
    /// Summary description for Salesdoctor_Bricks1
    /// </summary>
    ...
    public class Salesdoctor_Bricks1 : System.Web.Services.WebService
    {
        ...
        [WebMethod(EnableSession = true)]
        [ScriptMethod(ResponseFormat = ResponseFormat.Json)]
        public string GetSummaryProductSale( string ProductId, string doctorId, string From, string To)
        {
            ...
            try
            {
                ...
                _nvCollection.Add("@ProductId-varchar(max)", (ProductId == "-1" ? "0" : ProductId));
                _nvCollection.Add("@doctorId-varchar(50)", (doctorId)); //(doctorId == "-1" ? "0" : doctorId));
                _nvCollection.Add("@From-datetime", fromDateString);  //  From.ToString()
                _nvCollection.Add("@To-datetime", toDateString);  // To.ToString()


                // DataSet ds = GetData("sp_NationalWiseBrickSaleReport", _nvCollection);
                DataSet ds = GetData("sp_GetDetail_DoctorIncentiveSales_Graph", _nvCollection);
        ...
        public string GetDoctor()
        {
            ...
            try
            {
                ...
                DataSet ds = GetData("AtcoCRM_V2_New.dbo.sp_GetAll_doctorsforsalesBricks", _nvCollection);
                returnstring = ds.Tables[0].ToJsonString();
            }
            catch (Exception ex)
            {
        ...
        [WebMethod(EnableSession = true)]
        [ScriptMethod(ResponseFormat = ResponseFormat.Json)]
        public string GetProduct(string Dcotor_id)
        {
            ...
            try
            {
                ...
                _nvCollection.Add("@Dcotor_id-int", Dcotor_id);
                DataSet ds = GetData("AtcoCRM_V2_New.dbo.sp_GetAll_doctorsProduct_forsalesBricks", _nvCollection);
                returnstring = ds.Tables[0].ToJsonString();
            }
            catch (Exception ex)
            {
