...
function FillMainFormGrid() {
    ...
    $.ajax({
        type: "POST",
        url: "FileStructureService.asmx/GetDistributors",
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: onSuccessGridDistributor,
        error: onError,
...
function onSuccessGridDistributor(data, status) {
    var tabledata = "";

    $('#DistList').empty();
    $('#DistList').append('<table class="table table-striped table-bordered" id="FormsRecord">' +
            '<thead>' +
                '<tr>' +
                    '<th style="width: 50px;">S.No:</th>' +
                    '<th style="width: 200px;">Distributor Name</th>' +
                    
                    '<th style="width: 160px;">Add/View Structure</th>' +
                '</tr>' +
            '</thead>' +
    ...
    if (data.d != '') {
        ...
        for (var i = 0; i < jsonObj.length; i++) {
            ...
            tabledata += "<tr>" +
                    ...
                    "<td style='vertical-align: middle;'>" + jsonObj[i].Distributor + "</td>" +
                   
                    "<td style='vertical-align: middle;'>" +
                    "<button type='button' onclick='On_View_Structure(" + jsonObj[i].ID + ",&#39;" + jsonObj[i].Distributor + "&#39;)' class='btn btn-xs btn-info' data-toggle='tooltip' title='Add/View Structure'><i class='fa fa-plus'></i></button>" +
                  //  (" + jsonObj[i].ID + ", " + jsonObj[i].FormTime + ", \"" + jsonObj[i].TimeIn + "\", \"" + jsonObj[i].IsOneTimeScore + "\", " + jsonObj[i].DefaultScore + ", " + jsonObj[i].FormScore + ", \"" + jsonObj[i].FormName + "\")'
                    "</td>" +
                "</tr>";
        }
...
function On_View_Structure(Id,Name) {
    debugger;
    $.cookie('DistId', Id, { path: '/' });
   $.cookie('DistName', Name, { path: '/' });
    ...
    window.open('./ViewFileStructure.aspx', '_blank');
};
